...
MI_lead <- readr::read_csv(file='http://sldr.netlify.com/data/MI_lead.csv')
...
Tutorials can also serve as helpful reference guides later on, if you need reminders or more practice. We will keep using them to learn and practice R throughout the semester, and links to all our tutorials will be on the course Moodle page (Reference section).
...
```{r find-sqrt, exercise=TRUE, eval = FALSE}
function(information_R_needs)
```

```{r find-sqrt-hint-1, eval = FALSE}
...
```{r round-sqrt, exercise=TRUE, eval = FALSE}
function1(information_R_needs)
function2(information_R_needs)
```

```{r round-sqrt-hint-1, eval = FALSE}
sqrt(information_R_needs)
function2(information_R_needs)
```

```{r round-sqrt-hint-2, eval = FALSE}
sqrt(44)
function2(information_R_needs)
```

```{r round-sqrt-hint-3, eval = FALSE}
sqrt(44)
...
`c()` works great for creating small lists of just a few values, but it is not a good way to enter or store more than a few observations, or generate large data tables. Usually, we want to work with a *set* of variables that are stored together as a table so they can be used together, sorted into matching order, etc.  In R, these objects are called *tibbles* or *data.frame*s.  The next sections will get you started using them. 

## How should data tables be organized for statistical analysis?
A comprehensive guide to good practices for formatting data tables in spreadsheets for later analysis with statistical software is available at <http://kbroman.org/dataorg/>.
...
In this course, you will mainly need to use already-created datasets. As time goes on, you will learn to *modify* them if they were created without heed to the suggestions above, but you will probably not create many datasets yourself.
...
- `head()` prints out the first few rows of the dataset (6 rows, by default).
...
*Side note: the `::` lets you state which package contains a certain function you want to use, whether or not it's been loaded with `library()` or `require()`. So `readr::read_csv()` uses function `read_csv()` from package `readr`, whether `readr` is loaded or not.*
...
The code below should read in some baseball statistics from the file <https://sldr.netlify.app/data/baseball.csv>.

```{r baseball-mistakes, exercise = TRUE, eval = FALSE}
readr::read_csv(http://sldr.netlify.com/data/seball)
```

```{r baseball-mistakes-hint-1, eval = FALSE}
readr::read_csv(http://sldr.netlify.com/data/baseball)
```

```{r baseball-mistakes-hint-2, eval = FALSE}
readr::read_csv(http://sldr.netlify.com/data/baseball.csv)
```

```{r baseball-mistakes-hint-3, eval = FALSE}
readr::read_csv('http://sldr.netlify.com/data/baseball.csv')
```

```{r baseball-mistakes-hint-4, eval = FALSE}
 <- readr::read_csv('http://sldr.netlify.com/data/baseball.csv')
```

```{r baseball-mistakes-hint-5, eval = FALSE}
baseball <- readr::read_csv('http://sldr.netlify.com/data/baseball.csv')
```

### What about local files?
The same function, `readr::read_csv()`, can be used to read in a local file.  You just need to **change the input** to `readr::read_csv()` -- instead of a URL, you provide a path and filename (in quotes).  For example, the input file='http://sldr.netlify.com/data/MI_lead.csv' might become file='C:\\Data\\MI_lead.csv'.
...
The *input argument* we provided to `read_csv()` was the URL (in quotes -- either single or double quotes are fine).  But notice that this time, we gave the input argument a *name*, "file", and specified its value with an equal sign.

This is not *required* - the command works fine without it:

```{r}
MI_lead <- readr::read_csv('https://sldr.netlify.app/data/MI_lead.csv')
```

However, if a function has *more than just one* input argument, it's good to get in the habit of providing names for the inputs.  

- If you provide names, then the order in which you list the inputs doesn't matter; 
- Without names, **the order matters**! You may have to use ? to figure out what order R expects.
...
This is an advanced topic, so don't worry if it seems complicated; for now, it just nice to realize some of the power R has to clean up and reorganize data. We will learn more later.
...
```{r, rename-MI_lead-repeat, echo=TRUE}
MI_lead <- MI_lead %>%
...
The code above used some tools you've seen, and some more advanced ones you haven't seen yet.  

One important one is the pipe, "%>%", which can be read as "AND THEN...". It takes the output from one function and "sends" it to the next one in the chain. I imagine the data and results as water flowing in a pipe from one function to the next.

For more details and a rationale for why using pipes is a smart way to write code (*optional read!*), see [a helpful guide  to pipes from the University of Cinncinnati](https://uc-r.github.io/pipe).

### Back to our renaming
...
Translated into words, the code tells R:
...
"Finally, show me a `glimpse()` of my new dataset."
...
### Datasets in R
You've considered several different ways to get datasets to work with in R: you can use datasets that are built in to R or R packages, or you can use `readr::read_csv()` to read in data files stored in .csv format.
