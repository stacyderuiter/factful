...
```{r, gf_point, eval = FALSE, exercise = TRUE}
...
```{r, manual-order-boxplots, eval = FALSE, exercise = TRUE}
university_teachers <- university_teachers %>%
...
```{r, change-denom-hint-1, eval = FALSE}
gf_props( ~ TVHrsDay, fill = ~ Marijuana, data = NHANES,
          denom = ...)
```

```{r, change-denom-hint-2, eval = FALSE}
gf_props( ~ TVHrsDay, fill = ~ Marijuana, data = NHANES,
          denom = ~...)
```

```{r, change-denom-hint-3, eval = FALSE}
gf_props( ~ TVHrsDay, fill = ~ Marijuana, data = NHANES,
          denom = ~ x)
```
...
```{r mmr-hint-2, eval = FALSE}
mmr <- read_csv('http://sldr.netlify.app/data/MammalMetabolicRates.csv')
gf_bar(_____, data = _____)
```

```{r mmr-hint-3, eval = FALSE}
mmr <- read_csv('http://sldr.netlify.app/data/MammalMetabolicRates.csv')
gf_bar(_____, data = mmr)
```


```{r mmr-hint-4, eval = FALSE}
mmr <- read_csv('http://sldr.netlify.app/data/MammalMetabolicRates.csv')
...
```{r cleveland-dot-hint-1, eval = FALSE}
...
```{r, color-map-contin-hint-1, eval = FALSE}
gf_point(births ~ date, data = Births1978, 
         ...)
```


```{r, color-map-contin-hint-2, eval = FALSE}
